...
urlpatterns = patterns('',
    ...
    url(r'^post/(?P<post_id>.+)?$', PostResource.as_view(), name='post_resource'),
    url(r'^posts/(?P<post_id>.+)?$', PostResource.as_view(), name='post_resource'),
    # urls for post(s)/<post_id>/comments/
    url(r'^post/(?P<post_id>.+)/comments/?$',
    ...
    url(r'^author/(?P<author_id>.+)/posts/?$', VisiblePostToUser.as_view(),
        name='visibile_posts'),
    url(r'^author/(?P<author_id>.+)?$', AuthorProfile.as_view(),
        name='author_profile'),

    url(r'^friendrequest/$', csrf_exempt(FriendRequestView.as_view()),
        name='friend_request'),

    url(r'^friends/(?P<user_id_1>.+)/(?P<user_id_2>.+)/$',
        AreFriends.as_view(), name='are_friends'),
    url(r'^friends/?', FriendsView.as_view(),
        s, name='friends_view'),

    url(r'^test_rest/(?P<id>.+)/?$', front_views.test_rest, name="test_rest"),
)
